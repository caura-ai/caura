...
async def test_storage_mount_runs_migrations_inside_existing_lifespan(monkeypatch):
    events = []
    ...
    async with entry.app.router.lifespan_context(app):
        assert events == ["native startup", "bus migrations"]
    assert events == ["native startup", "bus migrations", "native shutdown"]
    assert app.openapi_schema is None
