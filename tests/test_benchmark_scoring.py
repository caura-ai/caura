...
def old_recall_boost(recall_count: int) -> float:
    ...
    return 1.0 + (RECALL_BOOST_CAP - 1.0) * recall_count / (
...
def new_recall_boost(
    ...
    return 1.0 + (RECALL_BOOST_CAP - 1.0) * recency * recall_count / (
        recall_count + RECALL_BOOST_SCALE
    )
...
class TestScoringLatency:
    ...
    def test_freshness_latency(self):
        ...
        old_result = _benchmark(lambda: old_freshness(s["created_at"], s["now"]))
        new_result = _benchmark(
            lambda: new_freshness(
                s["created_at"], s["ts_valid_start"], s["ts_valid_end"], s["now"]
            )
        )

        overhead_us = new_result["mean_us"] - old_result["mean_us"]
        print(f"\n{'─' * 60}")
        print("FRESHNESS DECAY (50K iterations)")
        print(
            f"  Old: mean={old_result['mean_us']:.2f}μs  p50={old_result['p50_us']:.2f}μs  p99={old_result['p99_us']:.2f}μs"
        )
        print(
            f"  New: mean={new_result['mean_us']:.2f}μs  p50={new_result['p50_us']:.2f}μs  p99={new_result['p99_us']:.2f}μs"
        )
        print(
            f"  Overhead: {overhead_us:+.2f}μs/call ({overhead_us / max(old_result['mean_us'], 0.01) * 100:+.1f}%)"
        )
        print(f"{'─' * 60}")

        # Sanity: new formula should add <5μs per call
        assert new_result["mean_us"] < 50, (
            f"New freshness too slow: {new_result['mean_us']:.1f}μs"
        )

    def test_recall_boost_latency(self):
        s = self._make_scenario()

        old_result = _benchmark(lambda: old_recall_boost(s["recall_count"]))
        new_result = _benchmark(
            lambda: new_recall_boost(
                s["recall_count"], s["last_recalled_at"], s["created_at"], s["now"]
            )
        )

        overhead_us = new_result["mean_us"] - old_result["mean_us"]
        print(f"\n{'─' * 60}")
        print("RECALL BOOST (50K iterations)")
        print(
            f"  Old: mean={old_result['mean_us']:.2f}μs  p50={old_result['p50_us']:.2f}μs  p99={old_result['p99_us']:.2f}μs"
        )
        print(
        ...
        assert new_result["mean_us"] < 50, (
            f"New recall boost too slow: {new_result['mean_us']:.1f}μs"
        )
    ...
    def test_full_score_latency(self):
        ...
        new_fresh = new_freshness(
            s["created_at"], s["ts_valid_start"], s["ts_valid_end"], s["now"]
        )
        new_rb = new_recall_boost(
            s["recall_count"], s["last_recalled_at"], s["created_at"], s["now"]
        )

        old_result = _benchmark(
            lambda: old_score(
                s["similarity"], s["weight"], old_fresh, s["entity_boost"], old_rb
            )
        )
        new_result = _benchmark(
            lambda: new_score(
                s["similarity"], s["weight"], new_fresh, s["entity_boost"], new_rb
            )
        )

        overhead_us = new_result["mean_us"] - old_result["mean_us"]
        print(f"\n{'─' * 60}")
        print("FULL SCORE (50K iterations)")
        print(
            f"  Old: mean={old_result['mean_us']:.2f}μs  p50={old_result['p50_us']:.2f}μs  p99={old_result['p99_us']:.2f}μs"
        )
        print(
        ...
        assert new_result["mean_us"] < 50, (
            f"New full score too slow: {new_result['mean_us']:.1f}μs"
        )
    ...
    def test_combined_pipeline_latency(self):
        ...
        print(f"\n{'═' * 60}")
        print("COMBINED PIPELINE (50K iterations)")
        print(
            f"  Old: mean={old_result['mean_us']:.2f}μs  p50={old_result['p50_us']:.2f}μs  p99={old_result['p99_us']:.2f}μs"
        )
        print(
        ...
        print("  Context: PG query latency is ~1,000-10,000μs")
        print("           Scoring overhead is <0.1% of total search latency")
        print(f"{'═' * 60}")

        # Combined pipeline should be under 100μs (trivial vs DB roundtrip)
        assert new_result["mean_us"] < 100, (
            f"Combined pipeline too slow: {new_result['mean_us']:.1f}μs"
        )
