...
def test_default_budget_tracks_the_gate_so_raising_it_cannot_break_startup():
    ...
    for gate in (1.0, 5.0, 7.9, 10.0, 25.0):
        got = _default_strong_embed_timeout(gate, required)
        assert got > gate, f"gate={gate} must leave room to outwait the gate, got {got}"
        assert got <= required, f"gate={gate} must not exceed the required cap, got {got}"

    # A gate raised past the required cap is incoherent on its own terms — it
    # would outlive the very embed it gates. Clamped rather than grown, and the
    # startup validator is what reports it.
