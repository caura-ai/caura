"""Unit tests for ``caura_manage`` (op: read | update | transition | delete | bulk_delete | lineage).
...
async def test_manage_invalid_op_errors(mcp_env):
    out = await mcp_server.caura_manage(op="wat", memory_id=VALID_UID)
...
async def test_manage_transition_missing_status_errors(mcp_env):
    out = await mcp_server.caura_manage(op="transition", memory_id=VALID_UID)
