...
and "IBM" was written and then destroyed inside a single batch.

Nothing raced, and that is the part worth stating. The loop is sequential and
the storage side applies updates in list order, so this is LAST-WINS and
perfectly reproducible: the same extraction loses the same alias every time. A
deterministic lost update is worse than a racy one in one specific way — it
never presents as flaky, so it never looks like something to investigate. It
just looks like the extractor only ever found one name.
...
def _worker_patches(fn):
    ...
    for deco in (
        ...
        patch(
            "core_api.services.entity_extraction_worker.upsert_relation",
            new_callable=AsyncMock,
        ),
    ):
        fn = deco(fn)
...
async def test_the_coalesced_form_still_resolves_as_a_relation_endpoint(
    ...
    assert mock_rel.await_count == 1, (
        "the relation stated via the coalesced form must still be upserted"
    )
    sent = mock_rel.await_args.args[0]
    assert str(sent.from_entity_id) == EXISTING_ID
