...
async def test_cross_link_threshold_is_applied_outside_the_limit():
    """caura#1616. The ONE test here that fails without the fix.

    Inside the ``LIMIT``, the threshold is a predicate the executor must
    satisfy before the limit is reached: it keeps pulling from the HNSW index
    until ten tuples clear the floor, and when few or none do that is a walk of
    the whole graph, once per candidate. Measured at ``lateral=143.1s`` for 100
    candidates, returning zero links.

    Outside, the limit is satisfiable from the first ten tuples and the scan
    stops. Since the rows are identical either way (see the two tests below),
    no behavioural assertion can tell the two forms apart — the placement is
    the entire change, so the placement is what is pinned."""
    from core_storage_api.services.postgres_service import PostgresService
    ...
    assert "LIMIT 10" in lateral, "the per-candidate cap must still be there"
    assert ">= :threshold" in lateral, "the similarity floor must still be applied"
    assert lateral.index("LIMIT 10") < lateral.index(">= :threshold"), (
        "the threshold must be applied OUTSIDE the LIMIT; inside it, the index "
        "scan cannot stop until ten rows clear the floor (caura#1616)"
    )


async def test_discover_caps_links_at_the_ten_nearest(sc):
    """Equivalence guard, not a mutation test — it passes on both forms.

    Twelve entities share the memory's embedding, so all twelve sit at
    similarity 1.0 and clear the floor. Both the old and new shapes must return
    exactly ten: this is the ">= 10 clear the floor" half of the prefix
    argument that makes moving the threshold safe."""
    tenant = _t()
...
async def test_discover_still_excludes_entities_below_the_threshold(sc):
    """The other half of the prefix argument, and the one that would catch a
    move of the floor that dropped it entirely.

    One entity matches the memory exactly (similarity 1.0); one is unrelated
    (fake_embedding puts it at ~0.12, well under the 0.75 floor). Only the
    match may be linked — filtering after the limit must still filter."""
    tenant = _t()
