...
*Last changed: 2026-09-17 (schema 1).*
...
| `version` | The core-api version string. | `CAURA_VERSION` env, `/app/VERSION`, package metadata, else `dev` |
...
| `providers.embedding` | `none`, `openai`, `local` or `other`. `fake` is reported as `none`. | `EMBEDDING_PROVIDER` |
| `providers.entity_extraction` | `none`, `openai`, `anthropic`, `openrouter`, `gemini` or `other`. `fake` is reported as `none`. | `ENTITY_EXTRACTION_PROVIDER` |
...
| `clients_24h.*` | Authenticated requests since the last accepted heartbeat, bucketed per client family, matched on the `User-Agent` prefix. Unknown agents fold into `other`; MCP requests count as `mcp` by transport. The raw header is never stored. | in-process counter |
...
no DNS lookup happens, and the client counter is never incremented. A test
patches `httpx.AsyncClient` and `asyncio.create_task` and asserts neither is
touched.
...
- **Block the host.** The only destination is `telemetry.caura.ai` on
  ...
  one attempt per day, a 5 second timeout, a debug-level log line, no retry.
...
- One attempt per cycle: `POST https://telemetry.caura.ai/api/telemetry/heartbeat`
  with a 5 second total timeout. Failure is logged at debug and the loop waits
  for the next cycle. No retry, no queue, no persistence of unsent payloads.
- Every replica of `core-api` runs the loop with the shared `deployment_id`;
  the collector keeps one heartbeat per deployment per 20 hours and drops the
  rest.
...
- The sender refuses a plain `http://` collector URL unless the host is
  `localhost`, so a mistyped override cannot send in clear text.

## Inspecting it

`GET /api/v1/telemetry` (normal API-key auth) returns the decision, the
reason if off, the deployment id, the collector URL, the last and next send
times, the last HTTP status, the one-line disable hint and
`payload_preview`: exactly what the next send would contain, built the same
way. When the heartbeat is off the preview is `null`: a disabled install does
no work, not even to show you what it would have sent.
