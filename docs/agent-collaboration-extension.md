...
Deployments that separately install compatible `caura-bus-core` and
`caura-bus-platform` packages can opt in by selecting these ASGI entrypoints:
...
The wrappers are Apache-2.0 integration code. The separately installed bus core
and platform packages retain their BUSL-1.1 license; opting in does not change
either license. The Enterprise dashboard/gateway change and the bus release
are deployment dependencies, not mandatory core dependencies.
