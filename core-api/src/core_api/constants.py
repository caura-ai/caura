...
def _default_strong_embed_timeout(gate_seconds: float, required_seconds: float) -> float:
    ...
    Clamped to ``required_seconds`` on the other side so the opportunistic budget
    never exceeds the *required* embed cap. A gate raised past that cap is
    incoherent on its own terms — it would outlive the embed it gates — and the
    startup validator says so rather than papering over it.
    """
    return min(
        required_seconds, max(_STRONG_EMBED_MIN_SECONDS, gate_seconds + _STRONG_EMBED_GATE_MARGIN_SECONDS)
    )
