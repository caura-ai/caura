"""Register the ``memclaw-insighter`` service agent for existing tenants."""  # legacy-name-floor: frozen migration data identity already present in deployed databases

# The automated nightly insights run (core-api
# ``lifecycle_audit._CoreApiLifecycleAdapter.insights``) now attributes its
# writes to a dedicated ``memclaw-insighter`` identity and self-registers that  # legacy-name-floor: describes the frozen deployed row identity
# agent the first time it runs for a tenant. This migration backfills the same
# registration for tenants that ALREADY hold ``memclaw-insighter`` memories (e.g.  # legacy-name-floor: identifies rows this frozen migration backfills
...
def upgrade() -> None:
    op.execute(
        ...
        SELECT DISTINCT m.tenant_id, 'memclaw-insighter', NULL, 'MemClaw Insighter', 'service', 3  -- legacy-name-floor: frozen deployed migration data values
        FROM memories m
        WHERE m.agent_id = 'memclaw-insighter'  -- legacy-name-floor: matches the frozen deployed row identity
...
def downgrade() -> None:
    ...
    op.execute(
        "DELETE FROM agents WHERE agent_id = 'memclaw-insighter'  -- legacy-name-floor: downgrade must target the frozen deployed row identity"
    )
