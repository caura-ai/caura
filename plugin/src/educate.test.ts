...
describe("writeEducationFiles", () => {
  ...
  describe("fenced-block migration", () => {
    ...
    test("user heading starting with the same prefix but no MemClaw content is left alone", () => { // legacy-name-floor: historical heading fixture
...
// MemClaw paragraph into each workspace's HEARTBEAT.md, fully // legacy-name-floor: historical on-disk paragraph
...
describe("shared SKILL.md (plugin/skills/memclaw/SKILL.md)", () => { // legacy-name-floor: shipped skill path
  // SKILL.md is no longer generated by a builder. It ships as a static file
  // at the plugin root (`plugin/skills/memclaw/SKILL.md`), discovered by // legacy-name-floor: shipped skill path
  ...
  test("suppresses slash-command exposure (user-invocable: false)", () => {
    ...
    assert.ok(
      ...
      "shared skill should set user-invocable: false to avoid a /memclaw slash command", // legacy-name-floor: OpenClaw skill slug
    );
  });

  test("gates on plugins.entries.memclaw.enabled via requires.config", () => { // legacy-name-floor: frozen plugin config key
    const skill = readSharedSkill();
    assert.ok(
  ...
  // gated on `plugins.entries.memclaw.enabled` and its footer references // legacy-name-floor: frozen plugin config key
...
describe("buildToolsMd", () => {
  ...
  test("points readers to the memclaw skill (by name, not by path) for the deep dive", () => { // legacy-name-floor: OpenClaw skill slug
    ...
    assert.ok(
      ...
      "TOOLS.md must point readers at the **memclaw** skill by name", // legacy-name-floor: OpenClaw skill slug
    );
  });
...
describe("buildAgentsMd", () => {
  ...
  test("nudges the model to open the memclaw skill before the first MemClaw call", () => { // legacy-name-floor: OpenClaw skill slug
    // The tool-reference deep dive lives in the memclaw skill, loaded // legacy-name-floor: OpenClaw skill slug
    ...
    // path (e.g. `skills/memclaw/SKILL.md`). The skill is no longer // legacy-name-floor: shipped skill path
    ...
    assert.ok(
      /\*\*memclaw\*\* skill/i.test(flat),
      "AGENTS.md must reference the **memclaw** skill by name", // legacy-name-floor: OpenClaw skill slug
    );
    assert.ok(
