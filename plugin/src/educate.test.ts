...
describe("writeEducationFiles", () => {
  ...
  // `includes("## Memory V2")`), so once a workspace had matching legacy
...
// onboarding paragraph into each workspace's HEARTBEAT.md, fully
...
describe("shared plugin-root SKILL.md", () => {
  ...
  test("suppresses slash-command exposure (user-invocable: false)", () => {
    ...
    assert.ok(
      ...
      "shared skill should set user-invocable: false to avoid exposing a slash command",
    );
  });
...
describe("buildToolsMd", () => {
  ...
  test("points readers to the bundled skill by name, not by path, for the deep dive", () => {
...
describe("buildAgentsMd", () => {
  ...
  test("nudges the model to open the bundled skill before the first Caura call", () => {
    // The tool-reference deep dive lives in the bundled skill, loaded
