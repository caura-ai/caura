...
For "how do I…", architecture sanity-checks, capacity questions, sharing what you built, or open-ended brainstorming — use [Discussions](https://github.com/caura-ai/caura/discussions) or chat with us on [Discord](https://discord.com/invite/aNfpgfpj). The issue tracker is reserved for actionable bug reports and concrete feature proposals.

## Bug reports

Open a [Bug report](https://github.com/caura-ai/caura/issues/new?template=bug_report.yml). The form requires the minimum fields we need to triage: version, deployment mode, OS, LLM provider, repro steps, and expected vs. actual behavior. Reports without those fields are slow to action.
...
Open a [Feature request](https://github.com/caura-ai/caura/issues/new?template=feature_request.yml). The form asks you to declare which surface from the SemVer contract the feature touches — this scopes the conversation early.
...
**Do not open a public issue.** Use [GitHub Security Advisories](https://github.com/caura-ai/caura/security/advisories/new) — reports are private to maintainers until a coordinated disclosure. Full policy in [SECURITY.md](SECURITY.md).
