...
The plugin is a TypeScript package in the `plugin/` directory of this repo. It claims the exclusive `memory` slot on an OpenClaw gateway, replacing the built-in `memory-core`, and provides 10 agent-facing tools, a ContextEngine with auto-read/write lifecycle, a heartbeat loop, and agent auto-education.
